...
export default function WorkerCompletion() {
  ...
  React.useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setError(ae.message || "Failed to load worker completion page");
      } finally {
        setPageLoading(false);
      }
    }
  ...
  async function handleCompleteJob() {
    ...
    try {
      await api(`/v1/jobs/${jobId}/complete`, {
        method: "POST",
        auth: true,
      });

      const refreshed = await api<JobDetailsResponse>(`/v1/jobs/${jobId}`, {
        auth: true,
      });
      ...
      if (ae.status === 403) {
        setError("Only the assigned worker can complete this job.");
        return;
      }

      if (ae.status === 409) {
        setError(ae.message || "Job cannot be completed from its current state.");
        return;
      }

      setError(ae.message || "Failed to complete job");
    } finally {
      setLoading(false);
    }
  }
  ...
  if (pageLoading) {
    return <div className="container mx-auto px-6 py-8">Loading worker completion page...</div>;
  }
    ...
    <div className="container mx-auto px-6 py-8">
      <div className="max-w-3xl mx-auto bg-white rounded-2xl border border-slate-200 p-8">
        ...
        <p className="text-slate-600 mt-1">
          Complete the job and review the customer.
        </p>
        ...
        {job && (
          <div className="mt-6 p-4 rounded-xl border border-slate-200 bg-slate-50">
            ...
            <div className="mt-3 text-sm text-slate-600">Customer</div>
            <div className="font-semibold text-slate-900">
              {job.createdBy?.name ?? "Customer"}
            </div>
            <div className="text-sm text-slate-600">
              Current reliability: {job.createdBy?.reliabilityScore ?? "?"}
            </div>
          </div>
        )}
        ...
        {job?.status !== "COMPLETED" && (
          <div className="mt-6">
            ...
            >
              {loading ? "Completing..." : "Mark Job Complete"}
            </button>
          </div>
        )}
              ...
              <form onSubmit={submitReview} className="mt-6 space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    rows={4}
                    className="mt-2 w-full border border-slate-200 rounded-xl px-4 py-3"
                    placeholder="How was the customer experience?"
                    value={notes}
